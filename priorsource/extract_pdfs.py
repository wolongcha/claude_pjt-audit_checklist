...
for rel in targets:
    ...
    if os.path.exists(txt_path):
        results.append(f"SKIP (exists): {rel}")
        continue
    ...
    try:
        ...
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(full_text)
        results.append(f"OK ({n}p, {len(full_text)}자): {rel}")
    except Exception as e:
        results.append(f"ERROR: {rel} -> {e}")

with open("_extract_log.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(results))
print("done, see _extract_log.txt")
